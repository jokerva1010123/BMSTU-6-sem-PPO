...
namespace DA
{
    public class ThingDA : IThingDB
    {
        ...
        {
            ConnectionCheck.checkConnection(this.Connector);
            string sql = getStrtTransferStudentThing(id_student, id_thing);
            NpgsqlCommand cmd = new NpgsqlCommand(sql, this.Connector);
            cmd.ExecuteNonQuery();
        }
        ...
        public string getStrtTransferStudentThing(int id_student, int id_thing)
        {
            return "update Things set id_student = " + id_student.ToString() + " where id_thing = " + id_thing.ToString() + ";";
        }
        public string getStrReturnThing(int id_thing)
        {
            return "update Things set id_student = -1 where id_thing = " + id_thing.ToString() + ";";
        }
    }
}
