...
namespace Main
{
	internal class App
	{
		...
		string MENU = "\n0.Выйти из программы.\n1.Выйти из аккаунта.\n2.Войти в аккаунт.\n3.Посмотреть список студентов.\n" +
			...
			"16. Добавить новое заявление.\n17. Изменить статус заявления.\n18. Посмотреть список всех заявлений\n19. Посмотреть список всех принятых заявлений\n" +
			"20. Посмотреть список всех непринятых заявлений\nВведите команду: ";
		public App(UserManager userManager, StudentManager studentManager, RoomManager roomManager, ThingManager thingManager, ReportManager reportManager)
		{
		...
		public void menu()
		{
			...
			switch (command)
			{
				...
				case 15:
					...
					break;
				case 16:
					if (this.role == Levels.STUDENT)
						this.reportManager.addReport();
					else
					{
						Console.WriteLine("Эту команду невозможно выполнить в текущем статусе!");
						log.Info("User has no right to add new report.");
					}
					break;
				case 17:
					if (this.role != Levels.NONE)
						this.reportManager.changeStatus();
					else
					{
						Console.WriteLine("Эту команду невозможно выполнить в текущем статусе!");
						log.Info("User has no right to change report's status.");
					}
					break;
				case 18:
					if (this.role != Levels.NONE)
						this.reportManager.viewAllReport();
					else
					{
						Console.WriteLine("Эту команду невозможно выполнить в текущем статусе!");
						log.Info("User has no right to view all reports.");
					}
					break;
				case 19:
					if (this.role != Levels.NONE)
						this.reportManager.viewAllDoneReport();
					else
					{
						Console.WriteLine("Эту команду невозможно выполнить в текущем статусе!");
						log.Info("User has no right to view all done reports.");
					}
					break;
				case 20:
					if (this.role != Levels.NONE)
						this.reportManager.viewAllNotDoneReport();
					else
					{
						Console.WriteLine("Эту команду невозможно выполнить в текущем статусе!");
						log.Info("User has no right to view all not done reports.");
					}
					break;
				default:
